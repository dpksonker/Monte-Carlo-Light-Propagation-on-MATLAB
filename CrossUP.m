function [Photon,Tissue] = CrossUP(Tissue,Photon,n,Radius_det,SM)
...
if ee <= Ref || Ref == 1                                                   % Check for reflection at boundary
    ...
    if Photon.Layer == 1                                                   % Check if the photon is at top-most layer                                 
        ...
        [Photon,Tissue] = Reflectance(Photon,Tissue,Radius_det,SM);                % Then call the function to check for diffuse reflectance
